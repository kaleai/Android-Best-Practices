...
public abstract class ExFragmentPagerAdapter extends FragmentPagerAdapter {
    ...
    public <E extends Fragment> boolean replaceFragment(int position, E fragment) {
        position = position % getCount();
        String tag = mFragmentTagSArr.get(position);
        if (tag != null) {
            FragmentTransaction ft = mFm.beginTransaction();
            ft.remove(mFm.findFragmentByTag(tag))
                    .add(fragment, tag)
                    .attach(fragment)
                    .commit();
            return true;
    ...
    public <E extends Fragment> boolean replaceFragment(Class<?> oldFragmentCls, E fragment) {
        for (int i = 0; i < mFragmentTagSArr.size(); i++) {
            if (getFragment(i).getClass().getName().equals(oldFragmentCls.getName())) {
